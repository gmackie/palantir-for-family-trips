...
function ActionChip({ icon: Icon, label, onClick, tone = "default" }) {
  const tones = {
    ...
    success:
      "border-[#3FB950]/30 bg-[#3FB950]/10 text-[#3FB950] hover:border-[#3FB950]",
    warning:
...
function getStopMeta(stop) {
  const items = [];
  if (stop.rating) {
    const reviews = stop.userRatingsTotal
      ? ` · ${stop.userRatingsTotal} reviews`
      : "";
...
function DriveStopEditor({ stop, onSelectEntity, onUpdateLocationFields }) {
  ...
  const photo = [...(stop.livePhotos || []), ...(stop.photos || [])].find(
    (item) => item?.imageUrl,
  );
        ...
        <div className="relative min-h-[196px] border-b border-[#30363D]/40">
          ...
          <div className="relative flex h-full flex-col justify-between p-4">
            <div className="flex items-start justify-between gap-3">
              <div
                className={`inline-flex h-12 w-12 shrink-0 items-center justify-center border shadow-[0_10px_24px_rgba(0,0,0,0.28)] ${visual.tone}`}
              >
                <StopIcon size={18} />
              </div>
              {photo?.label ? (
          ...
          {stop.address ? (
            <div className="flex items-start gap-3 border border-[#30363D] bg-[#11161d] px-3 py-3">
              ...
              <div>
                ...
                <div className="mt-1 text-[11px] leading-relaxed text-[#C9D1D9]">
                  {stop.address}
                </div>
              </div>
            </div>
        ...
        {isEditing ? (
          <div className="grid gap-2 border-t border-[#30363D]/50 bg-[#0b1016] p-4">
            <input
              value={stop.title || ""}
              onChange={(event) =>
                onUpdateLocationFields(stop.id, { title: event.target.value })
              }
...
export default function InspectorRail({
  ...
  const entity = useMemo(() => {
    ...
    return collectionName
      ? doc[collectionName]?.find((item) => item.id === selection.id) || null
      : null;
  }, [doc, selection]);
  ...
  const linkedEntities = useMemo(
    () => getLinkedEntities(doc, entity),
    [doc, entity],
  );
  const location = useMemo(
    () => getLocationForEntity(doc, entity),
    [doc, entity],
  );
  ...
  const routeOwner = useMemo(
    () =>
      entity?.type === "route"
        ? doc.families.find((family) => family.id === entity.familyId) || null
        : null,
    [doc.families, entity],
  );
  ...
  const lastEditedByFamily = useMemo(
    () =>
      doc.families.find(
        (family) => family.id === entity?.lastEditedByFamilyId,
      ) || null,
    [doc.families, entity?.lastEditedByFamilyId],
  );
  const prompts = useMemo(
    () => getDependencyPrompts(doc, entity),
    [doc, entity],
  );
  const driveStops = useMemo(() => {
    const stopIds = entity?.plannedStopIds || route?.stopLocationIds || [];

    const explicitStops = stopIds
      .map((locationId) =>
        doc.locations.find((location) => location.id === locationId),
      )
      .filter(Boolean);

    if (explicitStops.length) return explicitStops;

    return linkedEntities.filter(
      (item) => item.type === "location" && item.stopType,
    );
  }, [
    doc.locations,
    ...
    return (
      <aside className="flex w-[360px] flex-col border-l border-[#30363D] bg-[#161b22]">
        ...
        <div className="p-5 text-[11px] leading-relaxed text-[#8B949E]">
          Select any timeline block, map location, meal, activity, expense, or
          family to inspect and edit it here.
        </div>
      </aside>
    );
  }
  ...
  const detailRows = [
    ...
    "timeLabel" in entity && entity.timeLabel
      ? ["Timing", entity.timeLabel]
      : null,
    "origin" in entity && entity.origin ? ["Origin", entity.origin] : null,
    "originAddress" in entity && entity.originAddress
      ? ["Origin detail", entity.originAddress]
      : null,
    "driveTime" in entity && entity.driveTime
      ? ["Drive time", entity.driveTime]
      : null,
    "eta" in entity && entity.eta ? ["ETA", entity.eta] : null,
    "vehicle" in entity && entity.vehicle ? ["Vehicle", entity.vehicle] : null,
    "vehicleLabel" in entity && entity.vehicleLabel
      ? ["Call sign", entity.vehicleLabel]
      : null,
    "responsibility" in entity && entity.responsibility
      ? ["Task package", entity.responsibility]
      : null,
    "owner" in entity && entity.owner ? ["Owner", entity.owner] : null,
    "reservationType" in entity && entity.reservationType
      ? ["Mode", entity.reservationType]
      : null,
    "payer" in entity && entity.payer ? ["Payer", entity.payer] : null,
    "split" in entity && entity.split ? ["Split", entity.split] : null,
    "amount" in entity && entity.amount
      ? ["Amount", `$${entity.amount}`]
      : null,
    routeOwner?.originAddress
      ? ["Origin detail", routeOwner.originAddress]
      : null,
  ...
  const compactRailMode =
    compactStayMode || compactMealsMode || compactActivitiesMode;
  ...
  if (externalTarget?.externalUrl) {
    actionChips.push({
      ...
      label: "Open external",
      onClick: () =>
        window.open(externalTarget.externalUrl, "_blank", "noreferrer"),
    });
  }

  return (
    <aside
      className={`relative flex min-h-0 ${compactMealsMode || compactActivitiesMode ? "w-[320px]" : "w-[360px]"} flex-col overflow-hidden border-l border-[#30363D] bg-[#161b22]`}
    >
      <div
        className={`pointer-events-none absolute inset-x-0 top-0 z-10 h-1 bg-[#58A6FF] transition-all duration-500 ${
          recentlyUpdated
            ? "opacity-100 shadow-[0_0_22px_rgba(88,166,255,0.55)]"
            : "opacity-0"
        }`}
      />
      <div
      ...
      >
        ...
        <div className="mb-3 grid grid-cols-2 gap-2">
          <div className="border border-[#30363D] bg-[#0d1117] px-3 py-2 text-center">
            <div className="mb-1 text-[8px] font-black uppercase tracking-widest text-[#8B949E]">
              Checklist
            </div>
            <div className="text-[13px] font-black text-[#C9D1D9]">
              {taskCompletion}
            </div>
          </div>
          <div className="border border-[#30363D] bg-[#0d1117] px-3 py-2 text-center">
            ...
            <div className="text-[13px] font-black text-[#C9D1D9]">
              {entity.dayId ? entity.dayId.toUpperCase() : "ALL"}
            </div>
          </div>
        </div>
      ...
      >
        {familyDriveMode ? (
          <section className="border border-[#30363D] bg-[#161b22]">
            <div className="border-b border-[#30363D] px-4 py-3">
              <SectionTitle
                eyebrow="Road Trip"
                title="Inbound stop plan"
                meta={`${driveStops.length} stop${driveStops.length > 1 ? "s" : ""}`}
              />
            </div>
            <div className="border-b border-[#30363D]/30 px-4 py-4">
              ...
              <div className="mt-3 text-[11px] leading-relaxed text-[#8B949E]">
                {entity.routeSummary}
              </div>
            </div>
            {driveStops.map((stop) => (
        ...
        <section className="border border-[#30363D] bg-[#161b22] p-4">
          ...
          <div className="space-y-2 text-[11px] text-[#C9D1D9]">
            ...
            {("window" in entity && entity.window) ||
            ("timeLabel" in entity && entity.timeLabel) ? (
              <div className="flex items-center gap-2">
            ...
            {"summary" in entity && entity.summary ? (
              <div className="leading-relaxed text-[#8B949E]">
                {entity.summary}
              </div>
            ) : null}
            ...
            {"backup" in entity && entity.backup ? (
              <div className="rounded-[2px] border border-[#30363D] bg-[#0d1117] px-3 py-2 text-[#8B949E]">
                <span className="font-black uppercase tracking-wider text-[#D29922]">
                  Fallback:
                </span>{" "}
                {entity.backup}
              </div>
            ...
            <div className="mt-4 space-y-0 border-t border-[#30363D]/50 pt-3">
              {detailRows.length ? (
                ...
                <div className="text-[11px] text-[#8B949E]">
                  No additional logistics attached.
                </div>
              )}
            </div>
          ) : null}
        ...
        {(compactStayMode || compactActivitiesMode) && detailRows.length ? (
          <section className="border border-[#30363D] bg-[#161b22] p-4">
            <SectionTitle
              eyebrow="Details"
              title={
                compactActivitiesMode ? "Activity intel" : "Selected item intel"
              }
            />
            ...
            <div className="p-4">
              {prompts.length ? (
                <div className="mb-4 space-y-2">
                  {prompts.slice(0, 2).map((prompt) => (
                    <div
                      key={prompt.id}
                      className="rounded-[2px] border border-[#30363D] bg-[#0d1117] px-3 py-2 text-[11px] text-[#C9D1D9]"
                    >
              ...
              {tasks.length ? (
                ...
                <div className="mb-4 text-[11px] text-[#8B949E]">
                  No linked tasks yet. Add one below if this item needs
                  follow-up.
                </div>
              )}
          ...
          <section className="border border-[#30363D] bg-[#161b22] p-4">
            ...
            <div className="mb-3 space-y-2 text-[11px] text-[#C9D1D9]">
              ...
              {location.reservationNote ? (
                <div className="text-[#8B949E]">{location.reservationNote}</div>
              ) : null}
              {location.accessNote ? (
                <div className="text-[#8B949E]">{location.accessNote}</div>
              ) : null}
              {location.directionsNote ? (
                <div className="text-[#8B949E]">{location.directionsNote}</div>
              ) : null}
              {location.lockNote ? (
                <div className="text-[#8B949E]">{location.lockNote}</div>
              ) : null}
              {location.parkingNote ? (
                <div className="text-[#8B949E]">{location.parkingNote}</div>
              ) : null}
              {location.vehicleFee ? (
                <div className="text-[#8B949E]">
                  Gate fee: {location.vehicleFee}
                </div>
              ) : null}
              {location.wifiNetwork || location.wifiPassword ? (
                <div className="rounded-[2px] border border-[#30363D] bg-[#0d1117] px-3 py-2">
                  {location.wifiNetwork ? (
                    <div className="text-[#C9D1D9]">
                      WiFi: {location.wifiNetwork}
                    </div>
                  ) : null}
                  {location.wifiPassword ? (
                    <div className="mt-1 text-[#C9D1D9]">
                      Password: {location.wifiPassword}
                    </div>
                  ) : null}
                </div>
              ) : null}
              {location.hostName || location.coHostName ? (
                <div className="text-[#8B949E]">
                  ...
                  {location.coHostName
                    ? ` • Co-host: ${location.coHostName}`
                    : ""}
                </div>
              ) : null}
              {location.guestSummary ? (
                <div className="text-[#8B949E]">
                  Guests: {location.guestSummary}
                </div>
              ) : null}
              {location.confirmationCode ? (
                <div className="text-[#8B949E]">
                  Confirmation: {location.confirmationCode}
                </div>
              ) : null}
            </div>
            ...
            {location.manualUrl ? (
              <div className="flex gap-2">
                <ActionChip
                  ...
                  label="House manual"
                  onClick={() =>
                    window.open(location.manualUrl, "_blank", "noreferrer")
                  }
                />
              </div>
            ) : null}
        ...
        <section className="border border-[#30363D] bg-[#161b22] p-4">
          ...
          <textarea
            value={entity.note || ""}
            onChange={(event) =>
              onUpdateEntityNote(entity.type, entity.id, event.target.value)
            }
            placeholder={
              compactMealsMode
