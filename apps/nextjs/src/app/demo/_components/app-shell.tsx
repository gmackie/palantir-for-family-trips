...
const palantirLogo = "/demo/palantir-logo.svg";

import CommandMap from "./command-map";
import InspectorRail from "./inspector-rail";
import { isLiveExternalDataEnabled, PUBLISH_CONFIG } from "./publish-config";
import { DAYS, NAV_ITEMS, TIME_SLOTS, TRIP_META } from "./trip-data";
import {
...
import { usePersistedTripState } from "./use-persisted-trip-state";
import {
  fetchWeatherBundle,
  getMapWeather,
...
const SKIP_DEPRECATED_GOOGLE_ROUTING_IN_DEV =
  process.env.NEXT_PUBLIC_DISABLE_LEGACY_GOOGLE_ROUTING === "true";
const SKIP_DEPRECATED_GOOGLE_PLACES_IN_DEV = Boolean(
...
const VISIBLE_TIMELINE_SLOT_START =
  VISIBLE_TIMELINE_START_HOUR / TIMELINE_HOURS_PER_SLOT;
const VISIBLE_TIMELINE_SLOT_END =
  VISIBLE_TIMELINE_END_HOUR / TIMELINE_HOURS_PER_SLOT;
const VISIBLE_TIMELINE_SLOT_SPAN =
  VISIBLE_TIMELINE_SLOT_END - VISIBLE_TIMELINE_SLOT_START;
...
function getFamilyLabel(families, familyId) {
  return (
    families.find((family) => family.id === familyId)?.title || "Unknown family"
  );
}
...
function buildManualAllocationSeed(amount, families) {
  return Object.fromEntries(
    buildEqualExpenseAllocations(amount, families).map((item) => [
      item.familyId,
      item.amount,
    ]),
  );
...
function getFamilyExpenseBurden(expenses, families) {
  ...
  expenses.forEach((expense) => {
    ...
    getExpenseAllocations(expense, families).forEach((allocation) => {
      totals[allocation.familyId] =
        (totals[allocation.familyId] || 0) + allocation.amount;
    });
  });
...
function projectCursorToVisibleTimelineRatio(
  cursorSlot,
  dayCount = DAYS.length,
) {
  const normalizedCursor = clampTimelineCursor(cursorSlot);
  const dayIndex = Math.min(
    Math.max(Math.floor(normalizedCursor / TIME_SLOTS.length), 0),
    dayCount - 1,
  );
  const dayOffset = normalizedCursor - dayIndex * TIME_SLOTS.length;
  const clampedDayOffset = Math.min(
    Math.max(dayOffset, VISIBLE_TIMELINE_SLOT_START),
    VISIBLE_TIMELINE_SLOT_END,
  );
  const visibleCursor =
    dayIndex * VISIBLE_TIMELINE_SLOT_SPAN +
    (clampedDayOffset - VISIBLE_TIMELINE_SLOT_START);
  const totalVisibleSlots = Math.max(
    dayCount * VISIBLE_TIMELINE_SLOT_SPAN,
    0.0001,
  );
...
function projectVisibleTimelineRatioToCursor(ratio, dayCount = DAYS.length) {
  ...
  const dayIndex = Math.min(
    Math.max(Math.floor(visibleCursor / VISIBLE_TIMELINE_SLOT_SPAN), 0),
    dayCount - 1,
  );
  const dayVisibleOffset =
    visibleCursor - dayIndex * VISIBLE_TIMELINE_SLOT_SPAN;
  return clampTimelineCursor(
    dayIndex * TIME_SLOTS.length +
      VISIBLE_TIMELINE_SLOT_START +
      dayVisibleOffset,
  );
}
...
function getCursorHourInDay(cursorSlot) {
  ...
  const dayOffset =
    normalizedCursor -
    Math.floor(normalizedCursor / TIME_SLOTS.length) * TIME_SLOTS.length;
...
function getMissionLaunchCursor(dayIndex) {
  return clampTimelineCursor(
    dayIndex * TIME_SLOTS.length +
      MISSION_LAUNCH_HOUR / TIMELINE_HOURS_PER_SLOT,
  );
}

function getSuggestedPlaybackStartCursor(
  doc,
  cursorSlot,
  ...
  const activeWindow = windows.find(
    (window) =>
      normalizedCursor >= window.start && normalizedCursor <= window.end,
  );
  ...
  const nextCheckpoint = checkpoints.find(
    (checkpoint) => checkpoint.startSlot > normalizedCursor,
  );

  if (
    nextCheckpoint &&
    (!nextWindow || nextCheckpoint.startSlot <= nextWindow.start)
  ) {
    return clampTimelineCursor(
      Math.max(nextCheckpoint.startSlot - checkpointLeadIn, 0),
    );
  }
...
function getCurrentTripCursor(now = new Date()) {
  ...
  const tripDurationHours =
    (tripEnd.getTime() - tripStart.getTime()) / (1000 * 60 * 60);
  const hoursIntoTrip =
    (now.getTime() - tripStart.getTime()) / (1000 * 60 * 60);
...
function getRelatedTravelItemsForGate(doc, gate) {
  ...
  const gateEntityKeys = new Set(
    gateItems.flatMap((item) => [
      makeEntityKey("itineraryItem", item.id),
      ...(item.linkedEntityKeys || []),
    ]),
  );
  const sameDayTravelItems = doc.itineraryItems.filter(
    (item) => item.rowId === "travel" && item.dayId === primaryItem.dayId,
  );
  ...
  return sameDayTravelItems.filter(
    (item) =>
      item.startSlot >= gate.startSlot - 0.25 &&
      item.startSlot <= gate.startSlot + 0.55,
  );
}
...
function buildOperationGateContext(doc, gate) {
  ...
  const gateItemsWithType = gate.items.map((item) => ({
    ...item,
    type: "itineraryItem",
  }));
  ...
  const gateLocations = dedupeById(
    [
      ...
      ...relatedRoutes
        .map((route) =>
          getEntityById(doc, "location", route.destinationLocationId),
        )
        .filter(Boolean),
    ].filter(Boolean),
  );
  ...
  const familyIds = [
    ...
    ...relatedRoutes
      ...
      .filter((familyId) => familyId && familyId !== "all"),
  ];
  ...
  const etaSlot = relatedTravelItems.length
    ? Math.max(
        ...relatedTravelItems.map(
          (item) => item.startSlot + getItineraryItemEffectiveSpan(doc, item),
        ),
      )
    : gate.startSlot + getItineraryItemEffectiveSpan(doc, primaryItem);
  const etaLabel = stripDayPrefix(getSlotLabel(etaSlot));
  const participantLabel = !families.length
  ...
  const targetMeta = targetLocation
    ? getEntitySummary(targetLocation)
    : primaryItem.status || gate.subtitle;
  const routeCount =
    relatedRoutes.length || Math.max(relatedTravelItems.length, 1);
  const deploymentLabel = targetLocation
    ? `${participantLabel} deploying to ${targetTitle}.`
    : `${participantLabel} moving on ${gate.title}.`;
  const objective =
    MISSION_OBJECTIVE_COPY[dayId] ||
    `Advance ${participantLabel.toLowerCase()} into ${gate.title.toLowerCase()}.`;
...
function buildDailyBriefing(doc, context) {
  ...
  const activities = doc.activities
    ...
    .slice(0, 3);
  const tasks = getTasksForDay(doc, day.id)
    .filter((task) => task.status !== "done")
    .slice(0, 4);
  const liveItems = context.liveEntities.filter(
  ...
  return (
    <div className="mb-4">
      ...
      <div className="flex items-center justify-between gap-3">
        ...
        {meta ? (
          <div className="text-[10px] font-bold text-[#8B949E]">{meta}</div>
        ) : null}
      </div>
    </div>
  );
        ...
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-[#0b0f14]/86 backdrop-blur-sm">
          <div className="w-[420px] border border-[#30363D] bg-[#161b22] p-6 shadow-[0_24px_80px_rgba(0,0,0,0.45)]">
            ...
            <div className="mt-2 text-[12px] leading-relaxed text-[#8B949E]">
              This stays local in your browser, personalizes the planner to your
              family, and attributes edits and new expenses to you.
            </div>
        ...
        return (
          <button
            ...
            className={cn(
              ...
              selected
                ? "bg-[#24313d] shadow-[inset_4px_0_0_#58A6FF]"
                : "hover:bg-[#1f2a34]/60",
            )}
          >
            <div>
...
function ScenarioControls({
  ...
  const cursorDayIndex = Math.min(
    Math.max(Math.floor(clampedCursor / TIME_SLOTS.length), 0),
    DAYS.length - 1,
  );
  const selectedDay = DAYS[cursorDayIndex];
  const cursorHour = getCursorHourInDay(clampedCursor);
  const selectedHour = MISSION_TIME_PRESETS.reduce(
    (bestHour, hour) =>
      Math.abs(hour - cursorHour) < Math.abs(bestHour - cursorHour)
        ? hour
        : bestHour,
        ...
        {DAYS.map((day, dayIndex) => (
          <button
            ...
            onClick={() =>
              onSetCursor(
                dayIndex * TIME_SLOTS.length +
                  selectedHour / TIMELINE_HOURS_PER_SLOT,
              )
            }
            className={cn(
        ...
        {MISSION_TIME_PRESETS.map((hour) => {
          ...
          return (
            <button
              ...
              onClick={() =>
                onSetCursor(
                  cursorDayIndex * TIME_SLOTS.length +
                    hour / TIMELINE_HOURS_PER_SLOT,
                )
              }
              className={cn(
        ...
        <div className="border-b border-[#30363D] bg-[linear-gradient(135deg,rgba(88,166,255,0.08),rgba(13,17,23,0.95)_58%)] px-6 py-5">
          <div className="mb-4 flex items-start justify-between gap-4">
            <div>
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <span
                  className={`border px-2 py-1 text-[9px] font-black uppercase tracking-[0.18em] ${toneStyles[briefing.tone] || toneStyles.Blue}`}
                >
                  {briefing.code}
                </span>
              </div>
          ...
          <div className="grid gap-4 xl:grid-cols-[1.25fr_0.75fr]">
            <div className="border border-[#30363D] bg-[#0d1117]/85 p-4">
              ...
              <p className="text-[13px] leading-7 text-[#C9D1D9]">
                {briefing.summary}
              </p>
            </div>
            <div className="grid gap-3 sm:grid-cols-3 xl:grid-cols-1">
              <div className="border border-[#30363D] bg-[#0d1117] p-4 text-center">
                <div className="text-[8px] font-black uppercase tracking-widest text-[#8B949E]">
                  Live now
                </div>
                <div className="mt-2 text-[22px] font-black text-[#F0F6FC]">
                  {briefing.liveItems.length}
                </div>
              </div>
              <div className="border border-[#30363D] bg-[#0d1117] p-4 text-center">
                <div className="text-[8px] font-black uppercase tracking-widest text-[#8B949E]">
        ...
        <div className="grid max-h-[calc(100vh-220px)] gap-5 overflow-y-auto p-6 xl:grid-cols-[1.05fr_0.95fr]">
          <div className="space-y-5">
            <div className="border border-[#30363D] bg-[#161b22] p-4">
              ...
              <div className="space-y-3">
                {briefing.lookouts.map((item) => (
                  <div
                    key={item}
                    className="border border-[#30363D] bg-[#0d1117] px-3 py-3 text-[11px] leading-relaxed text-[#C9D1D9]"
                  >
                    {item}
                  </div>
                ))}
            ...
            {railSection(
              "Live surfaces",
              briefing.liveItems,
              "Nothing is active yet for this day window.",
            )}
            {railSection(
              "Coming up",
              briefing.soonItems,
              "No immediate follow-ups are queued right now.",
            )}
          </div>
            ...
            <div className="border border-[#30363D] bg-[#161b22] p-4">
              ...
              <div className="space-y-3">
                {briefing.activities.map((activity) => (
                  ...
                  >
                    <div>
                      <div className="text-[11px] font-bold uppercase tracking-[0.08em] text-[#C9D1D9]">
                        {activity.title}
                      </div>
                      <div className="mt-1 text-[10px] text-[#8B949E]">
                        {activity.window}
                    ...
                    <StatusPill tone={activity.status}>
                      {activity.status}
                    </StatusPill>
                  </button>
                ))}
                ...
                {!briefing.activities.length && !briefing.meals.length ? (
                  <div className="text-[11px] text-[#8B949E]">
                    No day-specific beats are attached yet.
                  </div>
                ) : null}
              </div>
            </div>
            ...
            <div className="border border-[#30363D] bg-[#161b22] p-4">
              ...
              <div className="space-y-2">
                {briefing.tasks.length ? (
                  ...
                  <div className="text-[11px] text-[#8B949E]">
                    No open day-specific tasks. Good hunting.
                  </div>
                )}
              </div>
            </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-hidden bg-[#03070b]/28 px-6 py-8 backdrop-blur-[2px]">
      ...
      <div
        ...
        style={{
          ...
          animation:
            "mission-launch-panel-in 420ms cubic-bezier(0.2, 0.8, 0.2, 1)",
        }}
      >
        <div
          className="absolute inset-0"
              ...
              {statusCards.map((card) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="uppercase tracking-[0.14em] text-[#8B949E]">
                      {card.label}
                    </span>
          ...
          <div className="flex items-center justify-center">
            ...
            >
              ...
              <div className="relative text-center">
                <div className="text-[8px] font-black uppercase tracking-[0.22em] text-[#8B949E]">
                  Launch In
                </div>
                <div
                  key={countdownValue}
                  ...
                  style={{
                    animation: "mission-launch-digit-in 220ms ease-out",
                  }}
                >
                  {countdownValue}
                </div>
...
function MissionFeedTray({ items, onActivateItem }) {
  ...
  return (
    <div
      className="pointer-events-none absolute bottom-4 right-4 z-40 w-[320px]"
      aria-live="polite"
      aria-atomic="false"
    >
          ...
          return (
            <button
              ...
              style={{
                ...
                background:
                  "linear-gradient(180deg, rgba(17,22,29,0.96), rgba(11,16,24,0.94))",
...
function SituationBoard({ context, onOpenEntity, onOpenBriefing }) {
  const sections = [
    {
      title: "Live now",
      items: context.liveEntities,
      emptyLabel: "Nothing active in this window.",
    },
    {
      title: "Coming up",
      items: [...context.nextEntities, ...context.prepSoon].slice(0, 4),
      emptyLabel: "No immediate follow-ups.",
    },
  ];
  ...
  return (
    <div className="border border-[#30363D] bg-[#161b22]">
      <div className="border-b border-[#30363D] bg-[#1f2a34]/30 p-5">
        ...
        <div className="text-[18px] font-black text-[#F0F6FC]">
          {context.cursorLabel}
        </div>
          ...
          <div key={section.title}>
            ...
            <div className="overflow-hidden border border-[#30363D] bg-[#0d1117]">
              {section.items.length ? (
                ...
                <div className="px-4 py-3 text-[11px] text-[#8B949E]">
                  {section.emptyLabel}
                </div>
              )}
            </div>
...
function TimelineBoard({
  ...
  const rowLayouts = rows.map((row, index) => ({
    ...
    top: rows
      .slice(0, index)
      .reduce((sum, item) => sum + (rowHeights[item.id] || 40), 0),
  }));
  const timelineHeight = rowLayouts.reduce((sum, row) => sum + row.height, 0);
  const familyLaneMap = new Map(
    doc.families.map((family, index) => [family.id, index]),
  );
  const actualTimelineRatio = projectCursorToVisibleTimelineRatio(
    getCurrentTripCursor(liveNow),
    days.length,
  );
  ...
  const hoverCursorLabel =
    hoverCursorSlot == null ? null : getSlotLabel(hoverCursorSlot);
  const cursorRatio = projectCursorToVisibleTimelineRatio(
    cursorSlot,
    days.length,
  );
        ...
        <div className="flex w-28 flex-col justify-center gap-1 border-r border-[#30363D] bg-[#0d1117]/50 px-2">
          ...
          <div className="flex items-center justify-center gap-1">
            ...
            <button
              ...
              onClick={() => {
                const currentIndex =
                  PLAYBACK_SPEED_OPTIONS.indexOf(playbackSpeed);
                const nextSpeed =
                  PLAYBACK_SPEED_OPTIONS[
                    (currentIndex + 1) % PLAYBACK_SPEED_OPTIONS.length
                  ];
      ...
      <div className="flex" style={{ height: `${timelineHeight}px` }}>
        ...
        >
          <div className="absolute inset-0">
            {Array.from({ length: days.length * visibleHoursPerDay + 1 }).map(
              (_, index) => {
                ...
                return (
                  <div
                    ...
                    className={cn(
                      ...
                      isMajor
                        ? "border-l border-[#30363D]/32"
                        : "border-l border-[#30363D]/10",
                    )}
                    style={{
                      left: `${(index / (days.length * visibleHoursPerDay)) * 100}%`,
                    }}
          ...
          <div className="absolute inset-0">
            {rowLayouts.map((row) => {
              const rowItems = doc.itineraryItems.filter(
                (item) => item.rowId === row.id,
              );
              const laneCount =
                row.id === "travel" ? Math.max(doc.families.length, 1) : 1;
              ...
              return (
                ...
                >
                  ...
                  {rowItems.map((item) => {
                    ...
                    const itemDayIndex = Math.min(
                      Math.max(
                        Math.floor(item.startSlot / TIME_SLOTS.length),
                        0,
                      ),
                      days.length - 1,
                    );
                    ...
                    const laneIndex =
                      row.id === "travel"
                        ? (familyLaneMap.get(item.familyIds?.[0]) ?? 0)
                        : 0;
                    const itemTop =
                      row.id === "travel" ? laneIndex * laneHeight + 2 : 6;
                    const itemHeight =
                      row.id === "travel" ? laneHeight - 4 : row.height - 12;
                    const selected =
                      selection.type === item.type && selection.id === item.id;
                    const compactTravelItem =
                      row.id === "travel" && itemSpan <= 0.22;
                    const shortTravelItem =
                      row.id === "travel" && itemSpan <= 0.42;
                    const compactTravelLabel = compactTravelItem
                      ? getCompactTravelLabel(item)
                      : null;

                    const itemWidthPercent =
                      (projectCursorToVisibleTimelineRatio(
                        clippedEnd,
                        days.length,
                      ) -
                        projectCursorToVisibleTimelineRatio(
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          ...
                          selected
                            ? "ring-1 ring-white/80 shadow-[0_0_0_1px_rgba(255,255,255,0.15)]"
                            : "",
                        ...
                        {row.id === "travel" ? (
                          <span
                            className={cn(
                              ...
                              compactTravelItem ? "justify-center" : "",
                            )}
                          >
                            {compactTravelItem ? null : (
                              <CarFront size={10} className="shrink-0" />
                            )}
                            <span
                              className={cn(
          ...
          {hoverCursorSlot != null ? (
            <div
              ...
              style={{
                left: `${projectCursorToVisibleTimelineRatio(hoverCursorSlot, days.length) * 100}%`,
              }}
            >
          ...
          {days.map((day, dayIndex) => (
            <div key={day.id} className="relative flex flex-1 flex-col">
              ...
              <div className="flex h-full">
                {Array.from({ length: visibleHoursPerDay }).map(
                  (_, hourOffset) => {
                    const hour = VISIBLE_TIMELINE_START_HOUR + hourOffset;
                    const hourCursor = clampTimelineCursor(
                      dayIndex * TIME_SLOTS.length +
                        hour / TIMELINE_HOURS_PER_SLOT,
                    );
                    const showLabel =
                      (hour - VISIBLE_TIMELINE_START_HOUR) % 3 === 0;
                    const isActive =
                      Math.abs(cursorSlot - hourCursor) <
                      1 / TIMELINE_HOURS_PER_SLOT / 2;
...
function IntelAction({ icon: Icon, label, onClick, tone = "default" }) {
  const tones = {
    default:
      "border-[#30363D] bg-[#0d1117] text-[#C9D1D9] hover:border-[#58A6FF]/40 hover:text-[#58A6FF]",
    amber:
      "border-[#D29922]/30 bg-[#D29922]/10 text-[#D29922] hover:border-[#D29922]",
  };
  ...
  return (
    <div className="flex items-start gap-2 text-[11px]">
      ...
      <div className="min-w-0">
        ...
        <div
          className={muted ? "mt-0.5 text-[#8B949E]" : "mt-0.5 text-[#C9D1D9]"}
        >
          {value}
        </div>
      </div>
...
function getMealMedia(location) {
  const seen = new Set();
  return [...(location?.livePhotos || []), ...(location?.photos || [])].filter(
    (media) => {
...
const ACTIVITY_RESEARCH = {
  ...
  "sat-yosemite": {
    headline:
      "Yosemite should be structured around a few high-payoff, low-friction stops.",
    cards: [
      {
...
const JIANG_ROAD_TRIP_STOP_DEFAULTS = [
  {
    ...
    externalUrl:
      "https://www.google.com/maps/search/?api=1&query=Bravo+Farms+Kettleman+City",
    summary:
      "Good halfway lunch + restroom + leg-stretch stop on the LA inbound drive.",
    linkedEntityKeys: [
  ...
  },
  {
    ...
    externalUrl:
      "https://www.google.com/maps/search/?api=1&query=Oakdale+Cheese+%26+Specialties",
    summary:
      "Final reset stop before the mountain leg. Good for snacks, bathrooms, and a quick stretch.",
    linkedEntityKeys: [
      makeEntityKey("family", "north-star"),
      makeEntityKey("itineraryItem", "north-star-drive"),
    ],
...
const FAMILY_VEHICLE_DEFAULTS = {
  "north-star": {
    ...
    routeSummary:
      "Plan lunch in Kettleman City and a final light break in Oakdale before the last mountain leg into Pine Mountain Lake.",
  },
  "silver-peak": {
    ...
    routeSummary:
      "Plan a quick Oakdale Cheese reset stop before the final push into Pine Mountain Lake.",
  },
  "desert-bloom": {
    ...
    plannedStopIds: [],
    routeSummary:
      "Friday arrival push from Reno straight into Pine Mountain Lake.",
  },
};
...
const YOSEMITE_ROUTE_DEFAULTS = {
  ...
  externalUrl:
    "https://www.google.com/maps/search/?api=1&query=Big+Oak+Flat+Entrance+Yosemite",
  summary:
    ...
    <div className="border border-[#30363D] bg-[#0d1117] p-4">
      ...
      <div className="space-y-2">
        {bullets.map((bullet) => (
          <div
            key={bullet}
            className="text-[11px] leading-relaxed text-[#C9D1D9]"
          >
            {bullet}
          </div>
        ))}
...
function TransitStopCard({ stop, onSelectEntity }) {
  return (
    ...
    >
      ...
      <div className="mt-1 text-[12px] font-black uppercase tracking-[0.08em] text-[#C9D1D9]">
        {stop.title}
      </div>
      <div className="mt-2 text-[10px] leading-relaxed text-[#8B949E]">
        {stop.summary || stop.address}
      </div>
      {stop.address ? (
        <div className="mt-2 text-[10px] text-[#8B949E]">{stop.address}</div>
      ) : null}
    </button>
  );
}
...
function ItineraryPage({
  ...
  const context = useMemo(
    () => getTimelineContext(doc, effectiveCursorSlot),
    [doc, effectiveCursorSlot],
  );
  const dailyBriefing = useMemo(
    () => buildDailyBriefing(doc, context),
    [doc, context],
  );
  const operationCheckpoints = useMemo(
    () => buildOperationCheckpoints(doc),
    [doc],
  );
    ...
    const tick = () => {
      ...
      updateMissionFeedItems((current) => {
        const next = current.filter(
          (item) => now - (item.createdAt || 0) < expirationMs,
        );
  ...
  const handlePlaybackFeedItems = useCallback(
    (items) => {
      const nextItems = (Array.isArray(items) ? items : [items]).filter(
        Boolean,
      );
  ...
  const handleMissionFeedActivate = useCallback(
    (item) => {
  ...
  const armOperationCheckpointsFromCursor = useCallback(
    (cursorSlot) => {
      const normalizedCursor = clampTimelineCursor(cursorSlot);
      triggeredOperationCheckpointIdsRef.current = new Set(
        operationCheckpoints
          .filter(
            (checkpoint) => checkpoint.startSlot <= normalizedCursor + 0.001,
          )
  ...
  useEffect(() => {
    ...
    const animate = (timestamp) => {
      ...
      const nextCursor = clampTimelineCursor(
        currentCursor +
          deltaSeconds * PLAYBACK_SLOT_UNITS_PER_SECOND * playbackSpeed,
      );
  ...
  const handleTimelineCursorChange = useCallback(
    ...
    [
      armOperationCheckpointsFromCursor,
      clearMissionFeed,
      isPlaybackPlaying,
      onSetCursor,
    ],
  ...
  const handleTogglePlayback = useCallback(() => {
    ...
    const startingCursor = getSuggestedPlaybackStartCursor(
      doc,
      doc.ui.timeline.cursorSlot,
      operationCheckpoints,
    );
    ...
    console.info("[TripCommand] Playback started", {
      startingCursor,
      playbackSpeed,
    });
  }, [
  ...
  const handleRestartPlayback = useCallback(() => {
    ...
    console.info("[TripCommand] Playback restarted", {
      restartCursor,
      isPlaybackPlaying,
    });
    setOperationGate(null);
      ...
      <div className="grid h-full min-h-0 flex-1 grid-cols-[320px_minmax(0,1fr)] overflow-hidden">
        <div className="min-h-0 overflow-y-auto border-r border-[#30363D] bg-[#0d1117]">
          <div className="space-y-4 p-4">
            ...
            <div>
              <SectionTitle
                eyebrow="Response Plans"
                title="Travel units"
                meta={`${doc.families.length} families`}
              />
              <FamilyList
                doc={doc}
                selection={selection}
        ...
        <div className="grid min-h-0 min-w-0 grid-rows-[minmax(0,1fr)_auto] overflow-hidden">
          <div className="relative min-h-0 min-w-0 overflow-hidden">
            <CommandMap
              ...
              selectedLocationId={
                getLocationForEntity(doc, getEntityBySelection(doc, selection))
                  ?.id || null
              }
              selectedRouteId={
                getRouteForEntity(doc, getEntityBySelection(doc, selection))
                  ?.id || null
              }
  ...
  return (
    <div className="grid min-h-0 flex-1 grid-cols-[minmax(380px,440px)_1fr] overflow-hidden">
      <div className="overflow-y-auto border-r border-[#30363D] bg-[#161b22] p-6">
        <SectionTitle
          eyebrow="Basecamp"
          title={airbnb?.title || "Basecamp"}
          meta={TRIP_META.subtitle}
        />
        <SelectableCard
          ...
          className="mb-6 p-4"
        >
          <div className="mb-2 text-[10px] font-black uppercase tracking-widest text-[#8B949E]">
            Location
          </div>
        ...
        <div className="space-y-4">
          {doc.stayItems.map((item) => (
            ...
            >
              <div className="mb-2 flex items-center justify-between">
                <div className="text-[12px] font-black uppercase tracking-widest text-[#C9D1D9]">
                  {item.title}
                </div>
                <div className="text-[9px] font-black uppercase tracking-wider text-[#58A6FF]">
                  {item.category}
      ...
      <div className="overflow-y-auto bg-[#0d1117] p-6">
        <SectionTitle
          eyebrow="Basecamp Intel"
          title="Arrival, access, and house ops"
          meta="Visible without drill-in"
        />
        <div className="mb-6 grid gap-4 xl:grid-cols-[1.15fr_0.85fr]">
          <div className="border border-[#30363D] bg-[#161b22] p-4">
              ...
              <div className="border border-[#30363D] bg-[#0d1117] p-3">
                ...
                <div className="mt-1 text-[10px] text-[#8B949E]">
                  {isSanitizedStay
                    ...
                    : "Per vehicle at Pine Mountain Dr entrance"}
                </div>
              </div>
            </div>
            ...
            <div className="mt-4 space-y-3 border-t border-[#30363D]/50 pt-4 text-[11px] leading-relaxed text-[#8B949E]">
              <div>
                <div className="mb-1 text-[9px] font-black uppercase tracking-widest text-[#58A6FF]">
                  Arrival route
                </div>
            ...
            <div className="mb-3 flex items-center justify-between gap-3">
              ...
              {showExternalListing ? (
                <button
                  type="button"
                  onClick={() =>
                    window.open(airbnb.externalUrl, "_blank", "noreferrer")
                  }
            ...
            <div className="space-y-2">
              {showManual ? (
                <button
                  type="button"
                  onClick={() =>
                    window.open(airbnb.manualUrl, "_blank", "noreferrer")
                  }
                ...
                >
                  <div>
                    <div className="text-[10px] font-black uppercase tracking-widest text-[#58A6FF]">
                      House manual
                    </div>
                    <div className="mt-1 text-[11px] text-[#C9D1D9]">
                      Open the full guest handbook and rules
                    </div>
                  </div>
        ...
        <div className="mb-6 grid gap-4 md:grid-cols-2">
          {doc.families.map((family, index) => (
            ...
            >
              ...
              <div className="text-[12px] font-bold text-[#58A6FF]">
                {family.title}
              </div>
              <div className="mt-1 text-[10px] text-[#8B949E]">
                {family.headcount}
...
function MealsPage({
  ...
  const selectedTasks = getTasksForEntity(doc, selectedMeal)
    .filter((task) => task.status !== "done")
    .slice(0, 2);
  ...
  return (
    <div className="grid min-h-0 flex-1 grid-cols-[minmax(360px,0.78fr)_minmax(520px,1.22fr)] overflow-hidden">
      <div className="overflow-y-auto border-r border-[#30363D] bg-[#161b22] p-6">
        <SectionTitle
          eyebrow="Meal Logistics"
          title="Shared feeding plan"
          meta="Ownership + prep + kid friendliness"
        />
        <div className="space-y-3">
          {doc.meals.map((meal) => (
            <div
            ...
            >
              ...
              >
                <div>
                  <div className="font-bold text-[#8B949E]">
                    {getDayMeta(meal.dayId)?.shortLabel || meal.dayId}
                  </div>
                  <div className="mt-1 text-[12px] font-black text-[#C9D1D9]">
                    {meal.timeLabel}
                  </div>
                </div>
                <div className="min-w-0">
                ...
                <div className="space-y-2 text-right">
                  ...
                  <div className="text-[10px] text-[#8B949E]">
                    {formatMealTravelSignal(
                      meal,
                      getLocationForEntity(doc, meal),
                    )}
                  </div>
                </div>
              </button>
          ...
          <>
            <div className="border border-[#30363D] bg-[#161b22] p-5">
              <div className="mb-2 flex items-start justify-between gap-4">
                <div>
                  ...
                  <div className="mt-2 text-[11px] text-[#8B949E]">
                    {getDayMeta(selectedMeal.dayId)?.title} at{" "}
                    {selectedMeal.timeLabel} · {selectedMeal.reservationType}
                  </div>
                </div>
                <StatusPill tone={selectedMeal.status}>
                  {selectedMeal.status}
                </StatusPill>
              </div>
              ...
              <div className="mb-4 flex flex-wrap gap-2">
                ...
                {linkedMission ? (
                  <IntelAction
                    ...
                    onClick={() =>
                      onSelectEntity(linkedMission.type, linkedMission.id)
                    }
                    tone="amber"
                  />
                ) : null}
              ...
              <div className="grid gap-4 xl:grid-cols-[1.05fr_0.95fr]">
                <div className="border border-[#30363D] bg-[#0d1117] p-4">
                  <SectionTitle
                    eyebrow="Venue Intel"
                    title={selectedLocation?.title || "Venue pending"}
                    meta={ratingSummary}
                  />
                  <div className="space-y-3">
                    <InfoRow
                    ...
                    <InfoRow
                      ...
                      value={
                        selectedLocation?.reservationNote || selectedMeal.note
                      }
                      muted
                    />
                    <InfoRow
                      ...
                      value={
                        hoursPreview ||
                        "Opening hours will appear when place details are available."
                      }
                      muted={!hoursPreview}
                    />
                  </div>
                ...
                <div className="border border-[#30363D] bg-[#0d1117] p-4">
                  ...
                  <div className="space-y-3">
                    <InfoRow
                      ...
                      value={travelSummary}
                      muted={
                        !selectedLocation?.basecampDrive &&
                        selectedLocation?.id !== "pine-airbnb"
                      }
                    />
                    <InfoRow
                      ...
                      <div className="rounded-[2px] border border-[#30363D] bg-[#161b22] px-3 py-3">
                        ...
                        <div className="space-y-2">
                          {selectedTasks.map((task) => (
                            <div
                              key={task.id}
                              className="text-[11px] text-[#C9D1D9]"
                            >
                              {task.title}
                            </div>
                          ))}
            ...
            {media.length ? (
              <div className="mt-5 border border-[#30363D] bg-[#161b22] p-5">
                <SectionTitle
                  eyebrow="Visual Intel"
                  title="Venue references"
                  meta={`${media.length} asset${media.length > 1 ? "s" : ""}`}
                />
                <div className="grid gap-4 md:grid-cols-3">
                  {media.map((item) => (
                    <a
                      key={item.id}
                      href={
                        item.sourceUrl || selectedLocation?.externalUrl || "#"
                      }
                      target="_blank"
                      rel="noreferrer"
                      className="group overflow-hidden border border-[#30363D] bg-[#0d1117]"
...
function ActivitiesPage({
  ...
  const selectedActivity = useMemo(
    () =>
      selection.type === "activity"
        ? doc.activities.find((activity) => activity.id === selection.id) ||
          doc.activities[0]
        : doc.activities[0],
    [doc.activities, selection],
  );
  const selectedLocation = useMemo(
    () => getLocationForEntity(doc, selectedActivity),
    [doc, selectedActivity],
  );
  const linkedEntities = useMemo(
    () => getLinkedEntities(doc, selectedActivity),
    [doc, selectedActivity],
  );
  ...
  const research = selectedActivity
    ? ACTIVITY_RESEARCH[selectedActivity.id]
    : null;
  ...
  useEffect(() => {
    ...
    if (
      !transitFamilies.some(
        (entry) => entry.family.id === selectedTransitFamilyId,
      )
    ) {
    ...
    <div className="grid min-h-0 flex-1 grid-cols-[360px_minmax(560px,1fr)] overflow-hidden">
      <div className="overflow-y-auto border-r border-[#30363D] bg-[#161b22] p-6">
        <SectionTitle
          ...
          title="Day missions"
          meta={`${doc.activities.length} tracked`}
        />
        <div className="space-y-4">
          {doc.activities.map((activity) => (
            ...
            >
              <div className="mb-2 flex items-center justify-between">
                <h3 className="text-[13px] font-black uppercase tracking-widest text-[#C9D1D9]">
                  {activity.title}
                </h3>
                <StatusPill tone={activity.status}>
                  {activity.status}
              ...
              <p className="mb-3 text-[11px] leading-relaxed text-[#C9D1D9]">
                {activity.description}
              </p>
              <div className="border-t border-[#30363D]/50 pt-3 text-[10px] leading-relaxed text-[#8B949E]">
                <span className="font-black uppercase tracking-widest text-[#D29922]">
                  Fallback:
                </span>{" "}
                {activity.backup}
              </div>
        ...
        {selectedActivity ? (
          <>
            <div className="border border-[#30363D] bg-[#161b22] p-5">
              <div className="mb-3 flex items-start justify-between gap-4">
                <div>
                  ...
                  <div className="mt-2 text-[11px] text-[#8B949E]">
                    {getDayMeta(selectedActivity.dayId)?.title ||
                      selectedActivity.dayId}{" "}
                    · {selectedActivity.window}
                  </div>
                </div>
                <StatusPill tone={selectedActivity.status}>
              ...
              <div className="mb-4 flex flex-wrap gap-2">
                ...
                {selectedLocation ? (
                  <IntelAction
                    ...
                    onClick={() =>
                      onSelectEntity("location", selectedLocation.id)
                    }
                    tone="amber"
                  />
                ) : null}
              ...
              <div className="border border-[#30363D] bg-[#0d1117] p-4">
                ...
                <div className="space-y-3">
                  ...
                  <InfoRow
                    ...
                    value={selectedLocation?.title || "No anchor location set"}
                  />
                  <InfoRow
                    icon={Search}
                    label="Research read"
                    value={
                      ...
                      "Build the mission details for this day."
                    }
                    muted
                  />
                  <InfoRow
            ...
            {selectedActivity.id === "thu-transit" && selectedTransitPlan ? (
              <div className="mt-5 border border-[#30363D] bg-[#161b22] p-5">
                <SectionTitle
                  eyebrow="Transit Planning"
                  title="Family road trips"
                  meta={`${transitFamilies.length} active routes`}
                />
                <div className="mb-4 flex flex-wrap gap-2">
                ...
                <div className="grid gap-4 xl:grid-cols-[0.95fr_1.05fr]">
                  <div className="border border-[#30363D] bg-[#0d1117] p-4">
                    <SectionTitle
                      ...
                      title={selectedTransitPlan.family.title}
                      meta={selectedTransitPlan.family.eta}
                    />
                    <div className="space-y-3">
                      <InfoRow
                        icon={MapPin}
                        label="Origin"
                        value={selectedTransitPlan.family.origin}
                      />
                      <InfoRow
                        icon={Route}
                        label="Route read"
                        value={selectedTransitPlan.family.routeSummary}
                        muted
                      />
                      <InfoRow
                        icon={Users}
                        label="Vehicle / group"
                        value={`${selectedTransitPlan.family.vehicle} · ${selectedTransitPlan.family.headcount}`}
                      />
                    </div>
                    <div className="mt-4 flex flex-wrap gap-2">
                      <IntelAction
                      ...
                      <IntelAction
                        ...
                        label="Inspect route"
                        onClick={() =>
                          onSelectEntity("route", selectedTransitPlan.route.id)
                        }
                        tone="amber"
                      />
                    </div>
                  ...
                  <div className="border border-[#30363D] bg-[#0d1117] p-4">
                    <SectionTitle
                      eyebrow="Road-trip Stops"
                      title="Good break points"
                      meta={`${selectedTransitPlan.stops.length} planned`}
                    />
                    {selectedTransitPlan.stops.length ? (
                      ...
                      <div className="text-[11px] text-[#8B949E]">
                        No stop plan attached to this route yet.
                      </div>
                    )}
                  </div>
                </div>
...
function ExpensesPage({
  ...
  const activeExpenseId =
    ...
    doc.expenses.some((expense) => expense.id === selection.id)
      ? selection.id
      : doc.expenses[0]?.id;
  const activeExpense =
    doc.expenses.find((expense) => expense.id === activeExpenseId) || null;
  ...
  const total = useMemo(
    () => doc.expenses.reduce((sum, expense) => sum + expense.amount, 0),
    [doc.expenses],
  );
  ...
  const familyBurden = useMemo(
    () => getFamilyExpenseBurden(doc.expenses, doc.families),
  ...
  const payerMode =
    activeExpense && payerOptions.includes(activeExpense.payer)
      ? activeExpense.payer
      : "__custom__";
  ...
  useEffect(() => {
    ...
    setAmountDraft(
      activeExpense.amount === 0 ? "" : String(activeExpense.amount),
    );
    setCustomPayerDraft(
      payerMode === "__custom__" ? activeExpense.payer || "" : "",
    );
  ...
  const commitManualAllocationDraft = useCallback(
    (familyId) => {
      if (!activeExpense) return;
    ...
    <div className="grid min-h-0 flex-1 grid-cols-[minmax(420px,0.95fr)_minmax(440px,1.05fr)] overflow-hidden">
      <div className="overflow-y-auto border-r border-[#30363D] bg-[#161b22] p-6">
        ...
        <div className="mb-5 grid grid-cols-2 gap-3">
          <div className="border border-[#30363D] bg-[#0d1117] p-4">
            <div className="mb-1 text-[10px] font-black uppercase tracking-widest text-[#8B949E]">
              Total tracked
            </div>
            <div className="text-[20px] font-black text-[#C9D1D9]">
          ...
          <div className="border border-[#30363D] bg-[#0d1117] p-4">
            ...
            <div className="text-[20px] font-black text-[#D29922]">
        ...
        <div className="border border-[#30363D] bg-[#0d1117]">
          {doc.expenses.map((expense) => (
            ...
            >
              <div className="min-w-0 text-left">
                ...
                {expense.createdByFamilyId || expense.lastEditedByFamilyId ? (
                  <div className="mt-1 text-[9px] uppercase tracking-[0.12em] text-[#58A6FF]">
                    {expense.createdByFamilyId
                      ? `Created by ${getFamilyLabel(doc.families, expense.createdByFamilyId)}`
                      : ""}
                    {expense.createdByFamilyId && expense.lastEditedByFamilyId
                      ? " · "
                      : ""}
                    {expense.lastEditedByFamilyId
                      ? `Edited by ${getFamilyLabel(doc.families, expense.lastEditedByFamilyId)}`
                      : ""}
                  </div>
                ) : null}
              </div>
              ...
              <div className="font-mono text-[12px] text-[#C9D1D9]">
            ...
            <div className="mb-6 grid gap-4">
              <div className="grid gap-3 md:grid-cols-2">
                <label className="grid gap-1.5">
                  <span className="text-[9px] font-black uppercase tracking-[0.18em] text-[#8B949E]">
                    Expense title
                  </span>
                  <input
                    value={activeExpense.title || ""}
                    onChange={(event) =>
                      onUpdateExpenseFields(activeExpense.id, {
                  ...
                  <div className="grid gap-2">
                    <select
                      ...
                      onChange={(event) => {
                        ...
                        if (value === "__custom__") {
                          onUpdateExpenseFields(activeExpense.id, {
                            payer:
                              customPayerDraft || activeExpense.payer || "",
                          });
                          return;
                        }
                        onUpdateExpenseFields(activeExpense.id, {
                    ...
                    {payerMode === "__custom__" ? (
                      <input
                        ...
                        onChange={(event) =>
                          setCustomPayerDraft(event.target.value)
                        }
                        onBlur={() =>
                          onUpdateExpenseFields(activeExpense.id, {
                            payer: customPayerDraft.trim() || "Unassigned",
                          })
                        }
              ...
              <div className="border border-[#30363D] bg-[#161b22] p-4">
                <SectionTitle
                  ...
                  meta={
                    EXPENSE_SPLIT_LABELS[activeExpense.allocationMode] ||
                    activeExpense.split
                  }
                />
                <div className="mb-4 flex flex-wrap gap-2">
                  {[
                    ...
                    <button
                      ...
                      type="button"
                      onClick={() =>
                        onSetExpenseAllocationMode(activeExpense.id, mode.id)
                      }
                      className={cn(
                ...
                {activeExpense.allocationMode === "individual" ? (
                  <div className="text-[11px] leading-relaxed text-[#8B949E]">
                    This cost is marked as family-specific, so it does not
                    contribute to shared reimbursement totals.
                  </div>
                ) : (
                  <div className="grid gap-2">
                    {activeAllocations.map((allocation) => (
                      ...
                      >
                        <div>
                          ...
                          <div className="text-[10px] text-[#8B949E]">
                            {activeExpense.allocationMode === "equal"
                        ...
                        {activeExpense.allocationMode === "manual" ? (
                          <input
                            inputMode="decimal"
                            value={
                              manualAllocationDrafts[allocation.familyId] ?? ""
                            }
                            onChange={(event) =>
                              setManualAllocationDrafts((current) => ({
                                ...current,
                            ...
                            onBlur={() =>
                              commitManualAllocationDraft(allocation.familyId)
                            }
                            onFocus={(event) => event.target.select()}
                            placeholder="0"
                            className="border border-[#30363D] bg-[#161b22] px-3 py-2 text-[11px] text-[#C9D1D9] outline-none focus:border-[#58A6FF]"
                          ...
                          <div className="text-right text-[12px] font-black text-[#C9D1D9]">
                        ...
                        <div className="mt-2 flex items-center justify-between border border-[#30363D]/60 bg-[#0d1117] px-3 py-3">
                          <div>
                            ...
                            <div className="text-[12px] font-bold text-[#C9D1D9]">
                              {formatCurrency(manualAllocatedTotal)} /{" "}
                          ...
                          <button
                            type="button"
                            onClick={() =>
                              onResetExpenseAllocationsToEqual(activeExpense.id)
                            }
          ...
          <div className="grid gap-2">
            {familyBurden.map((entry) => (
              ...
              >
                ...
                <div className="text-[12px] font-black text-[#C9D1D9]">
...
function App() {
  const [doc, setDoc] = usePersistedTripState(
    TRIP_DOCUMENT_STORAGE_KEY,
    getInitialTripDocument(),
  );
  const [viewerProfile, setViewerProfile] = usePersistedTripState(
    VIEWER_PROFILE_STORAGE_KEY,
    { familyId: null },
  );
  ...
  const displayDoc = useMemo(
    () => projectTripDocument(doc, visibilityMode),
    [doc, visibilityMode],
  );
  ...
  const setActiveFamilyProfile = useCallback(
    (familyId) => {
  ...
  useEffect(() => {
    const jiangRoute = doc.routes.find(
      (route) => route.id === "route-la-north-star",
    );
    const jiangFamily = doc.families.find(
      (family) => family.id === "north-star",
    );
    const yosemiteLocation = doc.locations.find(
      (location) => location.id === "yosemite",
    );
    ...
    const needsVehicleRouteBackfill = doc.routes.some((route) => {
      ...
      return (
        ("simulationStartSlot" in defaults &&
          route.simulationStartSlot == null) ||
    ...
    const needsYosemiteBackfill =
      ...
      (yosemiteLocation.title !== YOSEMITE_ROUTE_DEFAULTS.title ||
        ...
        yosemiteLocation.coordinates?.lat !==
          YOSEMITE_ROUTE_DEFAULTS.coordinates.lat ||
        yosemiteLocation.coordinates?.lng !==
          YOSEMITE_ROUTE_DEFAULTS.coordinates.lng);
    ...
    setDoc((current) => {
      const nextLocations = [
        ...
        ...JIANG_ROAD_TRIP_STOP_DEFAULTS.filter(
          (stop) =>
            !current.locations.some((location) => location.id === stop.id),
        ),
      ].map((location) =>
      ...
      const nextFamilies = current.families.map((family) => {
        ...
        return {
          ...
          originCoordinates:
            family.originCoordinates || defaults?.originCoordinates,
          vehicleLabel: family.vehicleLabel || defaults?.vehicleLabel,
          plannedStopIds: family.plannedStopIds?.length
            ? family.plannedStopIds
            : defaults?.plannedStopIds || family.plannedStopIds,
          routeSummary:
            family.routeSummary ||
        ...
        current.routes.map((route) => {
          ...
          return {
            ...
            originCoordinates:
              ...
              defaults.originCoordinates ||
              route.path?.[0],
            path:
              route.path?.length > 1 && defaults.originCoordinates
                ? [
                    route.originCoordinates || defaults.originCoordinates,
                    ...route.path.slice(1),
                  ]
                : route.path,
            stopLocationIds: Array.isArray(route.stopLocationIds)
              ? route.stopLocationIds
              : (defaults.stopLocationIds ?? route.stopLocationIds),
            destinationLocationId:
              route.destinationLocationId ||
              defaults.destinationLocationId ||
              route.destinationLocationId,
            simulationStartSlot:
              "simulationStartSlot" in defaults
            ...
            simulationMilestones:
              "simulationMilestones" in defaults
                ? route.simulationMilestones?.length
                  ? route.simulationMilestones
                  : defaults.simulationMilestones
                : route.simulationMilestones,
          };
        }),
        nextLocations,
  ...
  useEffect(() => {
    ...
    const currentById = (collection) =>
      new Map(collection.map((item) => [item.id, item]));
    const collectionNeedsRefresh = (
      currentCollection,
      initialCollection,
      refreshIds,
    ) => {
      ...
      return [...refreshIds].some((id) => {
        ...
        return (
          !currentItem ||
          !initialItem ||
          JSON.stringify(currentItem) !== JSON.stringify(initialItem)
        );
      });
    };
    const missingRoutes = initialDoc.routes.filter(
      (route) =>
        !doc.routes.some((currentRoute) => currentRoute.id === route.id),
    );
    const missingItineraryItems = initialDoc.itineraryItems.filter(
      (item) =>
        !doc.itineraryItems.some((currentItem) => currentItem.id === item.id),
    );
    const hasObsoleteRoutes = doc.routes.some((route) =>
      OBSOLETE_PLAN_ROUTE_IDS.has(route.id),
    );
    const hasObsoleteItineraryItems = doc.itineraryItems.some((item) =>
      OBSOLETE_PLAN_ITINERARY_IDS.has(item.id),
    );
    const needsPlanRefresh =
      collectionNeedsRefresh(
        doc.families,
        initialDoc.families,
        SEEDED_PLAN_REFRESH_IDS.families,
      ) ||
      collectionNeedsRefresh(
        doc.locations,
      ...
      collectionNeedsRefresh(
        doc.meals,
        initialDoc.meals,
        SEEDED_PLAN_REFRESH_IDS.meals,
      ) ||
      collectionNeedsRefresh(
        doc.activities,
        initialDoc.activities,
        SEEDED_PLAN_REFRESH_IDS.activities,
      ) ||
      collectionNeedsRefresh(
        doc.tasks,
        initialDoc.tasks,
        SEEDED_PLAN_REFRESH_IDS.tasks,
      ) ||
      collectionNeedsRefresh(
        doc.routes,
        initialDoc.routes,
    ...
    if (
      ...
      !missingItineraryItems.length &&
      !hasObsoleteRoutes &&
      !hasObsoleteItineraryItems &&
      !needsPlanRefresh
    ) {
    ...
    setDoc((current) => {
      const syncCollection = (
        ...
        const initialMap = new Map(
          initialCollection.map((item) => [item.id, item]),
        );
        const filtered = currentCollection.filter(
          (item) => !obsoleteIds.has(item.id),
        );
        const existingIds = new Set(filtered.map((item) => item.id));
        const replaced = filtered.map((item) =>
          refreshIds.has(item.id) && initialMap.has(item.id)
            ? initialMap.get(item.id)
            : item,
        );
      ...
      const nextItineraryItems = syncCollection(
        [
          ...
          ...initialDoc.itineraryItems.filter(
            (item) =>
              !current.itineraryItems.some(
      ...
      const nextSelection =
        (current.selection?.type === "route" &&
          OBSOLETE_PLAN_ROUTE_IDS.has(current.selection.id)) ||
        (current.selection?.type === "itineraryItem" &&
          OBSOLETE_PLAN_ITINERARY_IDS.has(current.selection.id))
      ...
      return {
        ...
        families: syncCollection(
          current.families,
          initialDoc.families,
          SEEDED_PLAN_REFRESH_IDS.families,
        ),
        locations: nextLocations,
        meals: syncCollection(
          current.meals,
          initialDoc.meals,
          SEEDED_PLAN_REFRESH_IDS.meals,
        ),
        activities: syncCollection(
          current.activities,
          initialDoc.activities,
          SEEDED_PLAN_REFRESH_IDS.activities,
        ),
        tasks: syncCollection(
          current.tasks,
          initialDoc.tasks,
          SEEDED_PLAN_REFRESH_IDS.tasks,
        ),
  ...
  const timelineWeatherDays = useMemo(
    () =>
      DAYS.map((day) => ({
        ...day,
        ...getTripDayWeather(weatherState.targets, day),
      })),
  ...
  const selectEntity = useCallback(
    (type, id) => {
      setDoc((current) => {
        if (
          current.selection?.type === type &&
          current.selection?.id === id &&
          current.ui.searchQuery === ""
        ) {
          return current;
        }
      ...
      setDoc((current) => {
        const locations = current.locations.map((location) =>
          location.id === locationId
            ? stampFamilyMetadata({ ...location, ...patch }, currentFamilyId)
            : location,
        );
  ...
  useEffect(() => {
    ...
    const basecampLocation = doc.locations.find(
      (location) => location.id === "pine-airbnb",
    );
    const pendingPlaceLocations = doc.locations.filter((location) => {
      ...
      return (
        (needsPlaceMatch || needsPlaceDetails || needsDriveProfile) &&
        !locationIntelHydrationRef.current.has(location.id)
      );
    });
      ...
      try {
            ...
            placesService.findPlaceFromQuery(
              ...
              (results, status) => {
                if (
                  ...
                  reject(
                    new Error(
                      `Place match failed for ${location.id}: ${status}`,
                    ),
                  );
                  return;
                }
            ...
            placesService.getDetails(
              ...
              (result, status) => {
                if (
                  ...
                  reject(
                    new Error(`Place details failed for ${placeId}: ${status}`),
                  );
                  return;
                }
                resolve(result);
        ...
        for (const location of pendingPlaceLocations) {
          ...
          try {
            ...
            const livePhotos = (placeDetails?.photos || [])
              .slice(0, 3)
              .map((photo, index) => ({
                id: `${location.id}-live-photo-${index + 1}`,
                label:
                  index === 0 ? "Live venue photo" : `Venue photo ${index + 1}`,
            ...
            if (
              ...
              try {
                basecampDrive = await fetchDriveProfile(
                  basecampLocation.coordinates,
                  coordinates,
                );
              } catch {
            ...
            hydrateLocationDetails(location.id, {
              ...
              externalUrl: placeId
                ...
                : location.externalUrl,
              phoneNumber:
                placeDetails?.formatted_phone_number || location.phoneNumber,
              websiteUrl: placeDetails?.website || location.websiteUrl,
              rating: placeDetails?.rating || location.rating,
              userRatingsTotal:
                placeDetails?.user_ratings_total || location.userRatingsTotal,
              openingHours:
  ...
  useEffect(() => {
    ...
    const basecamp = doc.locations.find(
      (location) => location.id === "pine-airbnb",
    );
    const yosemite = doc.locations.find(
      (location) => location.id === "yosemite",
    );
    ...
    const loadWeather = async () => {
      try {
        const [basecampBundle, yosemiteBundle] = await Promise.all([
          fetchWeatherBundle({
            label: "Groveland Basecamp",
            coordinates: basecamp.coordinates,
          }),
          fetchWeatherBundle({
            label: "Yosemite West Entrance",
            coordinates: yosemite.coordinates,
          }),
        ]);
  ...
  const updateEntityNote = (type, id, value) => {
    setDoc((current) => {
      ...
      return {
        ...current,
        [collectionName]: updateEntityInCollection(
    ...
    const newActivity = stampFamilyMetadata(
      {
        ...
        description:
          description?.trim() ||
          "Custom activity stub. Define the actual plan, why it matters, and what the fallback looks like.",
        backup:
          "If this becomes too ambitious, downgrade to the easiest nearby alternative.",
        note: "",
      },
      currentFamilyId,
    );
      ...
      meals: current.meals.map((meal) =>
        meal.id === mealId
          ? stampFamilyMetadata(
              {
                ...meal,
                status: meal.status === "Assigned" ? "Pending" : "Assigned",
              },
              currentFamilyId,
            )
          : meal,
          ...
          return stampFamilyMetadata(
            {
              ...
              allocations:
                expense.allocationMode === "manual" &&
                expense.allocations &&
                Object.keys(expense.allocations).length
                  ? expense.allocations
          ...
          ? stampFamilyMetadata(
              {
                ...
                allocationMode: "manual",
                split: EXPENSE_SPLIT_LABELS.manual,
                allocations: buildManualAllocationSeed(
                  expense.amount,
                  current.families,
                ),
              },
      ...
      const newExpense = stampFamilyMetadata(
        {
          ...
          allocations: {},
          settled: false,
          linkedEntityKeys: currentFamilyId
            ? [makeEntityKey("family", currentFamilyId)]
            : [],
          note: "",
        },
        currentFamilyId,
    ...
    (cursorSlot) => {
      setDoc((current) => ({
        ...
        ui: {
          ...
          timeline: {
            ...current.ui.timeline,
            cursorSlot: clampTimelineCursor(cursorSlot),
          },
        },
      }));
    },
  ...
  const exportState = () => {
    const blob = new Blob([JSON.stringify(displayDoc, null, 2)], {
  ...
  if (displayDoc.selectedPage === "itinerary") {
    ...
    content = (
      <MealsPage {...pageProps} onToggleMealStatus={toggleMealStatus} />
    );
